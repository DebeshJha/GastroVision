...
filename='results_e'+str(max_epochs)+'_'+'b'+str(batch_size)+'_'+'lr'+str(lr)+'_'+'efficientnetB0'   #filename used for saving epoch-wise training details and test results 
...
class train:
    ...
    def train_net(self):
        ...
        #Initialize model
        model = torchvision.models.efficientnet_b0(weights=False).to(device)   # make weights=True if you want to download pre-trained weights
        
        
        model.load_state_dict(torch.load('./efficientnet_b0_rwightman-3dd342df.pth',map_location='cuda'))   # provide a .pth path for already downloaded weights; otherwise comment this line out 	        
        ...
        best_model=torchvision.models.efficientnet_b0(weights=False).to(device)
